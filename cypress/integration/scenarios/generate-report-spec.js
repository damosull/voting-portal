describe('Confirm votes against Recommendations captured in filter criteria', function () {
    ...
    it(`Generate Report`, function () {
        ...
        const configColumns = ['Agenda Key', 'Customer name', 'Job Number','Meeting Type'];


        cy.get('#btn-add-criteria').click({force:true})
        cy.get('#txt-filter-criteria').type('Policy')
        cy.get('input[value="Policy ID"]').check({force:true})
        cy.get('#btn-criteria-apply').click({force:true})
