...
describe('Generate Policy report,download and verify file headers', function () {
    ...
    it(`Generate Policy Report`, function () {
        ...
        cy.wait('@ReportType')
        cy.get('#report-adhoc-commands-container > div > select').select('Excel (.xls)').find(':selected').contains('Excel (.xls)')
        cy.get('#report-adhoc-commands-container > div > select').select('Excel (.xlsx)').find(':selected').contains('Excel (.xlsx)')
        ...
        cy.wait('@FileUpdate')
        cy.get('.scrollableContainer > ul  >li').find('span[data-bind="text: Name"]').then($name => {
